...
public class NetClient {
	...
	static {
		ActivityManager am = (ActivityManager) MyApplication.getmContext().getSystemService(Context.ACTIVITY_SERVICE);
		int memClass = am.getMemoryClass();
		int cacheSize = 1024 * 1024 * memClass / 4; // 硬引用缓存容量，为系统可用内存的1/4

		binner_options = new DisplayImageOptions.Builder().showImageOnLoading(R.drawable.em_large_logo_default_4) // resource
				// or
				.showImageForEmptyUri(R.drawable.em_large_logo_default_4) // resource
				.showImageOnFail(R.drawable.em_large_logo_default_4) // resource
																		// or
				...
				.build();
		icon_options = new DisplayImageOptions.Builder().showImageOnLoading(R.drawable.em_head) // resource
																								// or
				...
				.build();
		user_icon_options = new DisplayImageOptions.Builder().showImageOnLoading(R.drawable.em_head) // resource
				.showImageForEmptyUri(R.drawable.em_head) // resource
		...
		ImageLoaderConfiguration config = new ImageLoaderConfiguration.Builder(MyApplication.getmContext())
				.threadPoolSize(10).threadPriority(Thread.NORM_PRIORITY + 1)
				.tasksProcessingOrder(QueueProcessingType.FIFO).denyCacheImageMultipleSizesInMemory()
				.memoryCache(new LruMemoryCache(cacheSize)).memoryCacheSize(cacheSize)
				.discCache(new UnlimitedDiscCache(cacheDir)).discCacheSize(30 * 1024 * 1024).discCacheFileCount(500)
	 ...
	 * @param url
	 */
	public static void getHalfHeightBitmap(ImageView imageView, String uri, ImageLoadingListener lister) {
